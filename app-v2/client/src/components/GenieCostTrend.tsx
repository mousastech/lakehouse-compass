...
/** Evolutionary Genie cost chart: relative-window or specific-month period filter,
 * monthly (default) / daily granularity, from the precomputed genie_cost_trend. */
export function GenieCostTrend({ ws }: { ws: string }) {
  const t = useT();
  const q = useLiveRows('genie_cost_trend', '/api/rows/genie_cost_trend', ws);
  const [period, setPeriod] = useState<Period>('12m');
  const [grain, setGrain] = useState<Grain>('month');
  const [month, setMonth] = useState<string>('');

  // Daily points, summed by date across workspaces (Account mode returns many).
  const daily = useMemo<Pt[]>(() => {
    const m = new Map<string, number>();
    for (const r of q.rows) {
      const d = toStr(r.usage_date);
      if (!d) continue;
      m.set(d, (m.get(d) || 0) + toNum(r.billed_cost_usd));
    }
  ...
  // A specific month is inherently daily; otherwise honor the toggle.
  ...
  // Chart geometry (fixed viewBox, scales to container width).
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-card-foreground">{t('genie.cost.overTime')}</h3>
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-muted-foreground">{t('genie.cost.periodTotal')}: <span className="tnum font-semibold text-card-foreground">{usd(total)}</span></span>
